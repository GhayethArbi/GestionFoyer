...
public class ChambreServiceImpl implements IChambreService{
        ...
        if (!blocs.isEmpty()) {
            blocs.forEach(bloc -> {
                ...
                if (bloc.getChambres().isEmpty()) {
                    ...
                    bloc.getChambres().forEach(chambre -> System.out.println("NumChambre: " + chambre.getNumeroChambre() + " type: " + chambre.getTypeChambre()));
                }
                System.out.println("********************");
            });
        }
    ...
    //keywords
    @Override
    public List<Chambre> findAllChambreByTypeChambre(TypeChambre typeChambre){
        return chambreRepository.findAllByTypeChambre(typeChambre);
    }

    @Override
    public List<Chambre> retrieveByNumeroChambreBetween(long start, long end) {
        return chambreRepository.findByNumeroChambreBetween(start, end);
    }

    @Override
    public List<Chambre> retrieveAllByTypeChambreAndBloc_IdBloc(TypeChambre typeChambre, long id) {
        return chambreRepository.findAllByTypeChambreAndBloc_IdBloc(typeChambre, id);
    }

    @Override
    public List<Chambre> retrieveAllByReservationsIsEmpty() {
        return chambreRepository.findAllByReservationsIsEmpty();
    }
